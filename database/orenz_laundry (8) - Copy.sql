...
CREATE ALGORITHM=UNDEFINED DEFINER=`u1011496_orenz_laundry`@`localhost` SQL SECURITY DEFINER VIEW `vtotalberat`  AS  select sum(`detail_transaksi`.`berat`) AS `total_berat`,`detail_transaksi`.`id_transaksi` AS `id_transaksi` from `detail_transaksi` group by `detail_transaksi`.`id_transaksi` ;
